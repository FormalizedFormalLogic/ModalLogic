...
/-- Recursively builds, at each stage `m`, a witness world satisfying `P m`. -/
...
- [Fin74, Lemma 2] -/
lemma exists_strictChain {w₀ : M.World} (h0 : P 0 w₀)
    (step : ∀ (m : ℕ) w, P m w → ∃ v, w ≺ v ∧ ¬(v ≺ w) ∧ P (m + 1) v) :
...
lemma strictChain_rel_of_lt (hR : ∀ m : ℕ, u m ≺ u (m + 1)) {i j : ℕ} (hij : i < j) : u i ≺ u j := by
...
lemma strictChain_rel_of_le (hR : ∀ m : ℕ, u m ≺ u (m + 1)) {i j : ℕ} (hij : i ≤ j) : u i ≺ u j := by
  rcases Nat.lt_or_eq_of_le hij with hij | rfl
  · exact strictChain_rel_of_lt hR hij;
  · simp;

/-- A strict ascending chain has pairwise-distinct points.

- [Fin74, Lemma 1] -/
lemma strictChain_injective (hR : ∀ m : ℕ, u m ≺ u (m + 1)) (hs : ∀ m : ℕ, ¬(u (m + 1) ≺ u m)) :
