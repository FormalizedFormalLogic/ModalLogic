...
theorem LogicEMK.sound (h : A ∈ LogicEMK) {κ} [Nonempty κ] (F : Frame κ) [F.IsMonotonic]
  ...
  Hilbert.sound
    (by rintro _ (⟨_, _, rfl⟩ | ⟨_, _, rfl⟩)
        exacts [valid_axiomM_of_isMonotonic, valid_axiomK_of_hasPropertyK]) h

instance : (@LogicEMK α).Consistent :=
...
theorem LogicEK_ssubset_LogicEMK : @LogicEK ℕ ⊂ LogicEMK := by
  ...
  · intro h
    ...
    have : M.toFrame.HasPropertyK := ⟨fun X Y w hw => by
      simp only [Frame.box, M, Set.mem_inter_iff, Set.mem_setOf_eq, Set.mem_insert_iff,
    ...
    have h0 := LogicEK.sound hM M.toFrame M.Val 0
...
theorem LogicEM_ssubset_LogicEMK : @LogicEM ℕ ⊂ LogicEMK := by
  ...
  · intro h
    have hK : Axioms.K (.atom 0) (.atom 1) ∈ @LogicEM ℕ :=
