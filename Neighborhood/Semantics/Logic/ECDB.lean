...
theorem LogicECDB.consistent : (@LogicECDB α).IsConsistent := by
  by_contra! hC
  simpa using LogicECDB.sound frame_1_2 hC

instance : Nonempty (MaximalConsistentSet (@LogicECDB α)) :=
  MaximalConsistentSet.nonempty LogicECDB.consistent
