...
@[simp]
lemma frame_3_10528928.not_valid_axiomK [DecidableEq α] (hab : a ≠ b) :
    ¬frame_3_10528928 ⊧ (Axioms.K #a #b : Formula α) := fun h => by
  have h0 := h (fun c => if c = a then {0, 1} else if c = b then {0} else Set.univ) 1
